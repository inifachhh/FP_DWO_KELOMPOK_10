                                ...
                                <div class="card shadow mb-4">
                                    ...
                                    <div class="card-body-store">
                                        <div class="chart-area-store">
                                            ...
                                            <?php
                                                $mysqli = mysqli_connect("localhost", "root", "", "adventureworks2022");
                                                $vendor = mysqli_query($mysqli,"SELECT SUM(sf.order_qty) as total, p.category as category FROM sales_fact sf JOIN product p ON sf.product_id = p.product_id GROUP BY category");
                                                while($row = mysqli_fetch_array($vendor)){
                                                    $jenis_vendor[] = $row['category'];

                                                    $query = mysqli_query($mysqli,"SELECT SUM(sf.order_qty) as total, p.category as category FROM sales_fact sf JOIN product p ON sf.product_id = p.product_id GROUP BY category HAVING p.category='".$row['category']."'");
